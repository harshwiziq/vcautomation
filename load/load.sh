...
echo ' -d : duration: time in minutesi'
echo ' -h : hostname: like backend-vcqa.wiziq.com'
echo
exit 1;
}
...
URL='https://'
URL+=$HOST
URL+=':443/landing/session/v1/'
A=1
while [ $A -le $N ]; 
do
SESSNAME=$BASENAME$A
RES=$(./backend-create-class_load templates/class-basic-wiziq --class-id $SESSNAME --duration $DURATION --starts-in 10 --date $CLASSDATETIME -h $HOST)
