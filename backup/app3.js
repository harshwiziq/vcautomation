			...
			// this block is for info frames		
				 ...
				 if(Data.msg.info && Data.msg.info.name=="chat-box"){
						...
						if(ack !== 1) {       //ack frame is not recieved but some info packet is rec
							 ...
							 ws.send(JSON.stringify({"v":1,"seq":21,"type":"req","to":"content","from":"user:"+vc_id+".content:0","msg":{"command":"get-tmp-url","data":{"path":"/vctemp/"+encodedFilename,"name":fileName,"type":fileType,"user_id":"test@wiziq.com"}}}));								
						} 
				 ...
				 if(Data.from=="content") {
						if(l==1) {
							 ...
							 log.info({"url":Data.msg.data.access_url},'Access URL');
							 ...
							 log.info({'COMMAND':'curl --progress-bar '+args},'curl command is\n')
							 exec('curl --progress-bar '+args, function(error, stdout, stderr) {
									
									console.log('\n CURL COMMAND LOGS ARE');
									console.log('stdout\n'+stdout);
									console.log('stderr\n'+stderr);
									if(error !== null) {
										 ...
										 ws.send(JSON.stringify({"v":1,"seq":11,"type":"req","to":"content","from":"user:"+vc_id+".content:0","msg":{"command":"upload_complete","data":{"path":"/vctemp/"+encodedFilename,"name":fileName,"type":fileType,"size":fileSize,"url":access_url,"user_id":"test@wiziq.com","vc_id":vc_id,"u_name":"a","removeafter":3600,"tags":"content, "+fileExtension}}}));
						...
						if(l==2) {
							 ...
							 console.log('\n\n'+ JSON.stringify({"v":1,"seq":12,"type":"req","to":"content","from":"user:"+vc_id+".content:0","msg":{"command":"start-conversion","data":{"path":"/vctemp/"+encodedFilename,"name":fileName,"type":fileType,"size":247288,"url":access_url,"user_id":"test@wiziq.com","vc_id":vc_id,"u_name":"a","tags":"content, "+fileExtension}}}));
							 ws.send(JSON.stringify({"v":1,"seq":12,"type":"req","to":"content","from":"user:"+vc_id+".content:0","msg":{"command":"start-conversion","data":{"path":"/vctemp/"+encodedFilename,"name":fileName,"type":fileType,"size":247288,"url":access_url,"user_id":"test@wiziq.com","vc_id":vc_id,"u_name":"a","tags":"content, "+fileExtension}}}));
