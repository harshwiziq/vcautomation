...
describe('Session Cluster Test Suite [Single User Functionality]', function () {
	...
	it('verify vc_id should be same for every call to the same user', function(done){
		...
		for(var i=0;i<n;i++) {
			appcall(data.items[i].id, data.items[i].displayName).then(
			function(status) {
	...
	it('same user joined two different sessions verify info should be different (vc_id, chat room_id, sessionid)', function(done) {
			
		this.timeout(60000);
		log.info({"TC":'same user joined two different sessions verify info should be different (vc_id, chat room_id, sessionid)'},'Test Case');
		var Data={};
		function firstuser() {
