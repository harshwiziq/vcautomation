// This code is used for verifying co-attendee presence
//user1 comes in session, user2 comes in session, user1 should get user2 info as new-johny frame.
// firstuser method is used to register a new user and return the vc_id of this user to the test case
// seconduser method is used to register a second user and return the vc_id of attendee in the attendees array of session info frame
...
exports.firstuser=function(id,displayName,callback)
{

	var WebSocket=require('ws');
	var ws=new WebSocket(config.sessionurl);   // update the same session in the seconduser method
	...
	// this block is for info frames
			...
			if(! protocol.addressed_to_me(Data.to,vc_id)){
				return callback(0, null, ws, 'vc_id is not matching in the following frame\n'+data);
			}
			...
			/*	if(Data.msg.info.status !== 'ok') {
				...
				*/
				...
				if(ack != 1)        //ack frame is not recieved but some info packet is rec
					return callback(0, null, ws, 'ack frame is not recieved');
				else if(si !=1)    // sess-info frame is not recieved but chat frame is recieved
					return callback(0, null, ws, 'session info frame is not recieved');
				else if(av !=1)     // av-tokbox frame is not recieved
					return callback(0, null, ws, 'av-tokbox frame is not recieved');
			...
			if(Data.msg.info_id=="new-johnny"){    // new user joins the session
					...
					return callback(1, Data.msg.info[0].vc_id, ws, null);
...
exports.seconduser=function(id,displayName,callback)
{
	...
	ws.on('error',function(error){
                log.error({err:error},'Error occurs while connecting to web socket');
                return callback(0,'Error Occurs while connecting to websocket\n<b>'+error+'<b>');
        });
