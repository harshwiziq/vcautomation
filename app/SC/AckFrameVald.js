//This code is used for verifying full ack frame structure and validaions
	...
	/*	else {
			...
			/*	if(Data.msg.info.status !== 'ok') {
				...
				if(ack != 1)        //ack frame is not recieved but some info packet is rec
					return callback(0,'ack frame is not recieved');
				else if(si !=1)    // sess-info frame is not recieved but chat frame is recieved
					return callback(0,'session info frame is not recieved');
				else if(av !=1)     // av-tokbox frame is not recieved
					return callback(0,'av-tokbox frame is not recieved');
				else
