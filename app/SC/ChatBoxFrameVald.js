// This code is user for verifying full chat-box frmae structre and validations
	...
	// this block is for info frames
			...
			if(Data.msg.info.name=="chat-box"){
				...
				if(Data.msg.info.status !== 'ok') {
					 return  callback(0, ws, 'chat-box frame status is not ok\n'+data);
				}

				else {
					if(Data.msg.info.info.username.vc_id !== vc_id) {
						return callback(0, ws, 'vc_id in chat-box frame is not matching vc_id that was extracted from ack frame\n'+data);
				  	}
					if(Data.msg.info.info.root_url !== config.chatRootUrl) {
						return callback(0, ws, 'root_url is not matching in chat-box frame\n'+data);
					}
					if(! Data.msg.info.info.room_id) {
						return callback(0, ws, 'room_id is not exist in the chat-box frame\n'+data);
					 }
					else {
